...
const slideThemeInlineSchema = new mongoose.Schema(
  {
...
const sliderSlideSchema = new mongoose.Schema(
  {
    ...
    // Optional separate banner for mobile view
    mobileImageUrl: { type: String, default: "" },
    categoryId: { type: Number, default: null, index: true },
    theme: { type: slideThemeInlineSchema, default: undefined },
  },
  // `strict: false` so new fields (e.g. mobileImageUrl) never get dropped
...
function normalizeSlideImageUrl(value) {
  if (value === undefined || value === null) return "";
  const s = String(value).trim();
  // Store as string; allow empty to mean "not provided"
  return s.length > 4000 ? s.slice(0, 4000) : s;
}

function pickMobileImageUrlFromBody(body) {
  ...
  // Fallback: any key that looks like "mobile*image*"
  for (const [k, v] of Object.entries(b)) {
    const kk = String(k).toLowerCase();
    if (!kk.includes("mobile")) continue;
    if (!kk.includes("image") && !kk.includes("banner")) continue;
    if (typeof v === "string" && v.trim()) return v;
  }
  return "";
}

function pickMobileImageUrlFromReq(req) {
  const fromParsed = pickMobileImageUrlFromBody(req?.body);
  if (fromParsed) return fromParsed;
  const raw = typeof req?.rawBody === "string" ? req.rawBody : "";
  if (!raw) return "";
  try {
    const obj = JSON.parse(raw);
    return pickMobileImageUrlFromBody(obj);
  } catch {
    return "";
  }
}
...
app.post("/api/admin/slider", async (req, res) => {
  try {
    ...
    if (String(req.query?.debug || "") === "1") {
      const rawMobile = pickMobileImageUrlFromReq(req);
      return res.json({
        keys: Object.keys(req.body || {}),
        body: req.body,
        rawBody: req.rawBody || "",
        pickedMobileImageUrl: rawMobile,
      });
    }
    const {
      title,
    ...
    const rawMobileImageUrl = pickMobileImageUrlFromReq(req);
    ...
    const createPayload = {
      ...
      mobileImageUrl: normalizeSlideImageUrl(rawMobileImageUrl),
    ...
    const slideDoc = await SliderSlide.create(createPayload);

    // Ensure `mobileImageUrl` persists even if schema strictness differs in prod.
    // Use the underlying collection update as a fallback.
    if (createPayload.mobileImageUrl) {
      try {
        await SliderSlide.collection.updateOne(
          { _id: slideDoc._id },
          { $set: { mobileImageUrl: createPayload.mobileImageUrl } },
        );
      } catch {}
    }

    const out = slideDoc.toObject();
    if (createPayload.mobileImageUrl) out.mobileImageUrl = createPayload.mobileImageUrl;
...
app.put("/api/admin/slider/:id", async (req, res) => {
  try {
    ...
    if (req.body.subheading !== undefined) {
      setDoc.subheading = normalizeSlideSubheading(req.body.subheading);
    }
    if (req.body.theme !== undefined) {
      setDoc.theme = normalizeSlideTheme(req.body.theme);
    }

    // Prefer atomic update, but also force mobileImageUrl via raw collection as a fallback.
    await SliderSlide.updateOne({ id: slideId }, { $set: setDoc });
    if (Object.prototype.hasOwnProperty.call(setDoc, "mobileImageUrl")) {
      try {
        await SliderSlide.collection.updateOne(
          { id: slideId },
          { $set: { mobileImageUrl: setDoc.mobileImageUrl } },
        );
      } catch {}
    }

    const updated = await SliderSlide.findOne({ id: slideId }).lean();
